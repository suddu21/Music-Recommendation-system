...
def rec():
    ...
    with open ("song.txt", "r") as myfile:
        song=myfile.readlines()[0]
    #rint(song)
    year=int(sp.search(q=f'track: {song}',limit=1)['tracks']['items'][0]['album']['release_date'][:4])
    recs=recommend_songs([{'name': song, 'year':year}],  data)
    recs=pd.DataFrame(recs)
    recs.drop_duplicates()
    recs.index = np.arange(1, len(recs) + 1)
    print(recs.head())
    recs.to_csv('songs.csv',header=False)
